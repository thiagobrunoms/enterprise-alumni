...
class UseAsyncResult<Result> {
  ...
  UseAsyncResult({required this.running, required this.result, required this.error});
  ...
  UseAsyncResult<Result> copyWith({
    ...
    bool? running,
      ...
      UseAsyncResult<Result>(
          error: error ?? this.error, result: result ?? this.result, running: running ?? this.running);
}
...
class UseCallAsyncResult<Result, Params> {
  ...
  UseCallAsyncResult({required this.running, required this.result, required this.error, required this.call});
}
...
UseAsyncResult<Result> useAsync<Result>(Future<Result> Function() fn, List<Object?>? keys, {bool? clearResult}) {
  final state = useState<UseAsyncResult<Result>>(UseAsyncResult(running: false, result: null, error: null));

  useEffect(() {
    state.value = clearResult == true
        ? UseAsyncResult(running: true, result: null, error: null)
        : state.value.copyWith(running: true);
    var op = CancelableOperation.fromFuture(fn()).then((res) {
      state.value = state.value.copyWith(running: false, result: res);
    }, onError: (err, stacktrace) {
      state.value = state.value.copyWith(running: false, error: err);
    });
    return () => op.cancel();
  }, keys);
...
// }
//
UseCallAsyncResult<Result, Params> useCallAsync<Result, Params>(Future<Result> Function(Params params) fn,
    {List<Object?>? keys}) {
  final state = useState<dynamic>({'running': false, 'result': null, 'error': null});
  final call = useCallback((Params params) async {
    state.value = {'running': true, 'result': null, 'error': null};
    try {
      var res = await fn(params);
      state.value = {'running': false, 'result': res, 'error': null};
    } catch (err) {
      state.value = {'running': false, 'result': null, 'error': err};
    }
  }, keys ?? []);
